...
const fotoOptionen = [
  {
    id: "vorhanden",
    title: "Fotos sind vorhanden",
    preis: "+0 €",
    icon: Image,
  },
  {
    id: "gekauft-ki",
    title: "Gekaufte Bilder / KI Bilder verwenden",
    preis: "+0 €",
    icon: Sparkles,
  },
  {
    id: "fotoshooting",
    title: "Fotoshooting gewünscht",
    preis: "+490 € exkl. Anfahrt",
    icon: Camera,
  },
  {
    id: "videoshooting",
    title: "Videoshooting gewünscht",
    preis: "nach Aufwand",
    icon: Video,
  },
];
          ...
          <PageTransition>
            <div className="mb-8">
              <h1 className="text-2xl sm:text-3xl font-bold text-brand-primary leading-tight mb-2">
                Wie steht es um Fotos / Bilder für deine Website?
              </h1>
              <p className="text-sm text-brand-text-muted">Wähle eine Option.</p>
            </div>

            <div className="space-y-3 mb-10">
              {fotoOptionen.map((opt, i) => {
                const isSelected = state.fotos === opt.id;
                const Icon = opt.icon;
                return (
                  <motion.button
                    key={opt.id}
                    ...
                    onClick={() => updateState({ fotos: opt.id })}
